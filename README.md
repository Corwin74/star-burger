...
[Демо-версия сайта](https://star-burger.shockland.ru)
...
Вы можете определить переменную окружения `ROLLBAR_TOKEN` системы логирования [Rollbar](https://rollbar.com/). Когда речь зайдёт о SDK, выбирайте Django и получите инструкции по интеграции, в ней будет указан персональный токен. По умолчанию будет присвоено значение None и система логирования не будет активирована.
```sh
ROLLBAR_TOKEN=eb1c26e92fd745b484556f0e3325f080
```
Вы можете определить переменную окружения `ROLLBAR_ENVIRONMENT`, чтобы система логирования могла различать среды исполнения. По умолчанию будет присвоено значение `local`.
```sh
ROLLBAR_ENVIRONMENT=data_center_moscow
```
