...
```
Вы можете определить переменную окружения `ENVIRONMENT`, чтобы система логирования могла различать среды исполнения. По умолчанию будет присвоено значение `local`.
```sh
